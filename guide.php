...
$language_code = isset($_GET['language']) ? $_GET['language'] : '';

if (!empty($language_code)) {
    $stmt = $conn->prepare("SELECT gt.title as title, gt.content as content, categories.name as category_name, users.username as creator_username FROM guide_translations gt INNER JOIN guides ON gt.guide_id = guides.id INNER JOIN categories ON guides.category_id = categories.id INNER JOIN users ON guides.creator_id = users.id WHERE gt.guide_id = ? AND gt.language = ?");
    $stmt->bind_param("is", $guide_id, $language_code);
} else {
